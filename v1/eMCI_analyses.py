...
# add executive scores (file -containing each diag info- in eMCI folder to avoid duplicates)
df_executive = pd.read_excel("eMCI/score_ex.xlsx") 
...
# df_FS.to_excel('eMCI/computed_data/df_fs.xlsx')



# reload it
df_FS = pd.read_excel("eMCI/computed_data/df_fs.xlsx").set_index('Unnamed: 0')
...
df_pearson_AT.to_excel("eMCI/computed_data/df_pearson_AT.xlsx")
df_pearson_ATE = all_functions.compute_pearson(df_cleaned_ATE[labels])
df_pearson_ATE.to_excel("eMCI/computed_data/df_pearson_ATE.xlsx")
communities_AT = all_functions.compute_louvain_community(df_pearson_AT[labels])
np.save("eMCI/computed_data/communities_AT",communities_AT)
communities_ATE = all_functions.compute_louvain_community(df_pearson_ATE[labels])
np.save("eMCI/computed_data/communities_ATE",communities_ATE)
df_louvain_AT = all_functions.reorganize_with_louvain_community(df_pearson_AT[labels], communities_AT)
df_louvain_AT.to_excel("eMCI/computed_data/df_louvain_AT.xlsx")
df_louvain_ATE = all_functions.reorganize_with_louvain_community(df_pearson_ATE[labels], communities_ATE)
df_louvain_ATE.to_excel("eMCI/computed_data/df_louvain_ATE.xlsx")

all_functions.plot_matrice(df_pearson_AT, "pearson_AT", cmap=None, with_labels=False, saving_path="eMCI/figures/pearson_AT.png", show=0)
all_functions.plot_matrice(df_pearson_ATE, "pearson_ATE", cmap=None, with_labels=False, saving_path="eMCI/figures/pearson_ATE.png", show=0)
all_functions.plot_matrice(df_louvain_AT, "louvain_AT", cmap=None, with_labels=False, saving_path="eMCI/figures/louvain_AT.png", show=0)
all_functions.plot_matrice(df_louvain_ATE, "louvain_ATE", cmap=None, with_labels=False, saving_path="eMCI/figures/louvain_ATE.png", show=0)

df_mat_diff = all_functions.matrix_difference(df_cleaned_AT[labels], df_cleaned_ATE[labels], n_perm=1000)
df_mat_diff.to_excel("eMCI/computed_data/df_differences.xlsx")
