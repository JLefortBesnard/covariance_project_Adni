...
# get data to plot the executive score per high and low per diagnosis as boxplot
df_high = df[df['ADNI_EF'] >= df['ADNI_EF'].median()]
df_low = df[df['ADNI_EF'] < df['ADNI_EF'].median()]
dic['CN'] = [df_high['ADNI_EF'].values, df_low['ADNI_EF'].values]
...
df = clean_signal(df, nb_rois, labels, ['TIV', 'Age'])
